			...
			if (oauthcode == null ) {
				...
				try {
					authorization_request = OAuthClientRequest
							.authorizationLocation(authorization_location).setResponseType("code").setClientId(client_id).setScope(scopes).setState(java.util.UUID.randomUUID().toString())			
	...
	private String exchangeAuthorizationCodeForAccessToken(String oauthcode) {
		...
		try {
			accesstoken_request1 = OAuthClientRequest.tokenLocation(token_location)
					...
					.setClientSecret(clientsecret).setRedirectURI(redirecturl).setCode(oauthcode).buildQueryMessage();
		...
		log.info("Access token url being used: " + accesstoken_request1.getLocationUri());
		 ...
		 //Map<String, String> headers = Utils.getBasicAuthorizationHeader(client_id, clientsecret);
		   ...
		   // accesstoken_request1.setHeaders(headers);
		    ...
		    OpenIdConnectResponse oAuthResponse=null;
			
				oAuthResponse = oAuthClient1.accessToken(accesstoken_request1, OAuth.HttpMethod.POST, OpenIdConnectResponse.class);
